...
public class Intro_WebHit_Get_All_Tehsil {

    public static ResponseModel responseObject = null;
    private final AsyncHttpClient mClient = new AsyncHttpClient();

    public void getTehsil(final IWebCallback iWebCallback ,String _id) {

        String myUrl;


        myUrl = AppConfig.getInstance().getBaseUrlApi() + ApiMethod.POST.getTehsil + "?districtID="+_id;



        Log.d("LOG_AS", "getTehsil: " + myUrl + "  "  );

        mClient.setMaxRetriesAndTimeout(AppConstt.LIMIT_API_RETRY, AppConstt.LIMIT_TIMOUT_MILLIS);
        mClient.post(myUrl, new AsyncHttpResponseHandler() {
                    @Override
                    public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                        Log.d("LOG_AS", "onSuccess: "+statusCode);
                        String strResponse;
                        try {
                            Gson gson = new Gson();
